...
Split the data frame into 4 parts
```{r}
...
Train on 3/4 of data, test on 1/4, plot estimated models. The indexing is an absolute nightmare, as you can see, and this is the special case when n/k is an integer. Indexing with a minus sign first *removes* those indices.
```{r}
...
```{r}
linRMSEs <- sapply(1:k, 
                   function(i) rmse(faithful$eruptions[splits[testInds,i]],
                                    predict(linMods[[i]],
                                            faithful[splits[testInds,i],])))
